...
def allSetFiles(self, setListPath, dir_name, label):
        csvOut = []
        counter = 0
        with open(setListPath,'r') as set:
...
def generateDataFromFolder(set_list_path_healthy, set_list_path_blighted, folder_name_healthy, folder_name_blighted, isTraining, outDir):   
    ...
    if (isTraining):
        blighted_features = allFilesInDir(folder_name_blighted, 'B')
        healthy_features = allFilesInDir(folder_name_healthy, 'H')
        with open(outDir+"/"+"csvOut_train.csv",'w',newline='') as csvfile:
            obj = csv.writer(csvfile)
            obj.writerow(columnLabels)
            obj.writerows(blighted_features)
        with open(outDir+"/"+"csvOut_train.csv",'a',newline='') as csvfile:
            obj = csv.writer(csvfile)
            obj.writerows(healthy_features)
    else:
        features = allFilesInDir(folder_name, 'NA')
        with open(outDir+"/"+"csvOut_test.csv",'w',newline='') as csvfile:
            obj = csv.writer(csvfile)
            obj.writerow(columnLabels)
            obj.writerows(features)
            
def generate_data_from_sets(folder_name, healthy_set_path, blighted_set_path, isTraining, outDir):
    columnLabels = ('fileName','fvhu','fvhu2','fvhu3','fvhu4','fvhu5','fvhu6','fvhu7',
                    'fvha1','fvha2','fvha3','fvha4','fvha5','fvha6','fvha7','fvha7',
                    'fvha8','fvha9','fvha10','fvha11','fvha12',
                    'gray_mean', 'red_mean', 'green_mean', 'blue_mean', 'num_brown_red', 'num_brown_green', 
    ...
    if (isTraining):
        blighted_features = allSetFiles(blighted_set_path, folder_name, 'B')
        healthy_features = allSetFiles(healthy_set_path, folder_name, 'H')
        with open(outDir+"/"+"csvOut_train.csv",'w',newline='') as csvfile:
            obj = csv.writer(csvfile)
            obj.writerow(columnLabels)
            obj.writerows(blighted_features)
